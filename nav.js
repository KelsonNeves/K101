...
// ===== TAMANHOS (P / M / G) =====
// No produtos.json cada tamanho tem duas colunas: tamanhoP (medida em texto) + precoP (valor).
// Um tamanho só fica disponível se tiver preço válido (> 0).
const TAMANHOS = ['P', 'M', 'G'];
let produtoModalAtual = null;        // produto do modal aberto
let tamanhoSelecionadoModal = null;  // { letra, medida, preco, disponivel } do modal aberto
...
function obterTamanhos(produto) {
    return TAMANHOS.map(letra => {
        const preco = lerPreco(produto['preco' + letra]);
        const medida = (produto['tamanho' + letra] || '').toString().trim();
        return { letra, medida, preco, disponivel: preco !== null && preco > 0 };
    });
}

// P é sempre o padrão; se um produto estiver sem P, usa o primeiro tamanho disponível
function tamanhoPadrao(produto) {
    const lista = obterTamanhos(produto);
    return lista.find(t => t.letra === 'P' && t.disponivel) || lista.find(t => t.disponivel) || null;
}
...
function getProdutosFiltrados() {
    ...
    if (termoPesquisa.trim() !== '') {
        ...
        produtosFiltrados = produtosFiltrados.filter(p => {
            ...
            // Preço e medida de qualquer tamanho (P, M ou G)
            const termoNumero = parseFloat(termo.replace(',', '.'));
            const tamanhosProduto = obterTamanhos(p);
            const precoMatch = tamanhosProduto.some(t => {
                if (t.preco === null) return false;
                return t.preco.toFixed(2).replace('.', ',').includes(termo) ||
                       t.preco.toString().includes(termo) ||
                       (!isNaN(termoNumero) && t.preco === termoNumero);
            });
            const medidaMatch = tamanhosProduto.some(t => t.medida.toLowerCase().includes(termo));
            
            // Código do produto (se existir)
            const codigoMatch = p.cod && p.cod.toLowerCase().includes(termo);
            
            return nomeMatch || descricaoMatch || tagsMatch || precoMatch || medidaMatch || codigoMatch;
        });
    }
...
function renderizarProdutosPaginados() {
    ...
    produtosPagina.forEach(produto => {
        const badgeCorUnica = (produto.tipo === 'unico_material')
            ? '<span class="badge-cor-unica"><i class="fas fa-palette"></i> Escolha a cor</span>'
            : '';

        const tamanhoCard = tamanhoPadrao(produto);
        const precoCard = tamanhoCard ? `R$ ${tamanhoCard.preco.toFixed(2)}` : 'Consulte';
        ...
        html += `
            <div class="produto-card" data-id="${produto.id}">
                ...
                <div class="produto-info">
                    ...
                    <p class="produto-preco">${precoCard}</p>
                </div>
            </div>
        `;
    });
...
// ===== CATÁLOGO GLOBAL DE MATERIAIS (herdado por produtos tipo "unico_material") =====
// Estrutura carregada de materiais.json: { categoriaId: { nome, precoAdicional, cores: [{nome, codigo, textura}] } }
let materiaisDisponiveis = {};
// Lista "achatada" com todas as cores de todas as categorias, na ordem das categorias
// (cada categoria fica organizada em bloco, uma de cada vez, sem misturar com as outras)
let materiaisFlatList = [];
...
async function carregarMateriais() {
    try {
        ...
        materiaisFlatList = [];
        Object.entries(materiaisDisponiveis).forEach(([categoriaId, categoria]) => {
            categoria.cores.forEach(cor => {
                materiaisFlatList.push({
                    categoriaId: categoriaId,
                    categoriaNome: categoria.nome,
                    precoAdicional: categoria.precoAdicional,
                    nome: cor.nome,
                    codigo: cor.codigo,
                    textura: cor.textura
                });
...
// ===== SELETOR DE COR (produtos tipo "unico_material") =====
// Guarda o material/cor atualmente selecionado no modal aberto
let corSelecionadaModal = null;

// Gera o HTML do carrossel de bolinhas (com textura de cada material), agrupadas por categoria
function renderizarSeletorCores(lista) {
    if (!Array.isArray(lista) || lista.length === 0) return '';

    let categoriaAnterior = null;
    const bolinhasHTML = lista.map((m, i) => {
        // Marca visualmente o início de um novo bloco de categoria (sem criar seletor/aba)
        const novaCategoria = m.categoriaId !== categoriaAnterior;
        categoriaAnterior = m.categoriaId;

        return `
            <div class="cor-bolinha${novaCategoria && i > 0 ? ' inicio-categoria' : ''}"
                 data-categoria-id="${m.categoriaId}"
                 data-categoria-nome="${m.categoriaNome}"
                 data-nome="${m.nome}"
                 data-codigo="${m.codigo}"
                 data-textura="${m.textura}"
                 data-preco-adicional="${m.precoAdicional}"
                 style="background-image: url('${m.textura}'); background-color: ${m.codigo};"
                 title="${m.categoriaNome} - ${m.nome}">
            </div>
        `;
    }).join('');

    return `
        <div class="cor-selector-container">
            <p class="cor-selector-label">
                <i class="fas fa-palette"></i>
                <span id="cor-selecionada-categoria">${lista[0].categoriaNome}</span> —
                <span id="cor-selecionada-nome">${lista[0].nome}</span>
            </p>
            <div class="cor-carrossel-wrapper">
                <div class="cor-carrossel-marcador"></div>
                <div class="cor-carrossel" id="cor-carrossel">
                    ${bolinhasHTML}
                </div>
            </div>
        </div>
...
// Configura o comportamento do carrossel de cores após o modal ser inserido no DOM
function configurarSeletorCores() {
    const carrossel = document.getElementById('cor-carrossel');
    if (!carrossel) {
        corSelecionadaModal = null;
        return;
    }

    const bolinhas = Array.from(carrossel.querySelectorAll('.cor-bolinha'));
    const nomeSpan = document.getElementById('cor-selecionada-nome');
    const categoriaSpan = document.getElementById('cor-selecionada-categoria');

    function atualizarSelecao() {
        const containerRect = carrossel.getBoundingClientRect();
        ...
        bolinhas.forEach(bolinha => {
            ...
            const centroBolinha = rect.left + rect.width / 2;
            const distancia = Math.abs(centroBolinha - centerX);
        ...
        if (maisPerto) {
            maisPerto.classList.add('selecionada');
            corSelecionadaModal = {
                categoriaId: maisPerto.dataset.categoriaId,
                categoriaNome: maisPerto.dataset.categoriaNome,
                nome: maisPerto.dataset.nome,
                codigo: maisPerto.dataset.codigo,
                textura: maisPerto.dataset.textura,
                precoAdicional: parseFloat(maisPerto.dataset.precoAdicional) || 1
            };
            if (nomeSpan) nomeSpan.textContent = corSelecionadaModal.nome;
            if (categoriaSpan) categoriaSpan.textContent = corSelecionadaModal.categoriaNome;
        }
    }

    // Atualiza a seleção enquanto o usuário rola o carrossel (com debounce leve)
    let scrollTimeout;
    carrossel.addEventListener('scroll', () => {
        clearTimeout(scrollTimeout);
        scrollTimeout = setTimeout(atualizarSelecao, 80);
    });

    // Clicar em uma bolinha centraliza ela (e ela se torna a selecionada)
    bolinhas.forEach(bolinha => {
        bolinha.addEventListener('click', () => {
            bolinha.scrollIntoView({ behavior: 'smooth', inline: 'center', block: 'nearest' });
        });
    });

    // Seleção inicial (primeira cor da primeira categoria, já centralizada pelo padding do CSS)
    setTimeout(atualizarSelecao, 50);
}

// ===== INDICADOR DE ROLAGEM DO MODAL (seta + gradiente) =====
// Mostra a seta/gradiente enquanto houver conteúdo abaixo da área visível do
// modal-body e esconde assim que o usuário rolar até o último conteúdo.
function atualizarIndicadorScrollModal() {
    const indicador = document.getElementById('modal-scroll-indicator');
    if (!indicador || !modalBody) return;

    const faltaRolar = modalBody.scrollHeight - modalBody.scrollTop - modalBody.clientHeight;
    // Pequena margem de tolerância para evitar "piscar" por causa de arredondamento
    if (faltaRolar > 12) {
...
function abrirModalProduto(produtoId) {
    ...
    // Reseta a cor e o tamanho selecionados a cada abertura de modal (P sempre é o padrão)
    corSelecionadaModal = null;
    produtoModalAtual = produto;
    tamanhoSelecionadoModal = tamanhoPadrao(produto);

    const botoesTamanho = obterTamanhos(produto).map(t => `
        <button type="button"
                class="tamanho-btn${tamanhoSelecionadoModal && t.letra === tamanhoSelecionadoModal.letra ? ' selecionado' : ''}"
                data-tamanho="${t.letra}"
                ${t.disponivel ? '' : 'disabled title="Indisponível para este adesivo"'}
                onclick="selecionarTamanho('${t.letra}')">${t.letra}</button>
    `).join('');

    const textoTamanhoInicial = tamanhoSelecionadoModal
        ? (tamanhoSelecionadoModal.medida || `Tamanho ${tamanhoSelecionadoModal.letra}`)
        : 'Consulte-nos';
    const totalInicial = tamanhoSelecionadoModal ? tamanhoSelecionadoModal.preco : 0;

    const temSeletorCores = produto.tipo === 'unico_material' && materiaisFlatList.length > 0;

    modalBody.innerHTML = `
        <div class="modal-produto">
            <!-- Container da imagem com zoom -->
            <div class="imagem-container" id="imagem-zoom-container">
                <img src="${produto.imagem}" alt="${produto.nome}" id="modal-imagem" onerror="this.src='imagens/produtos/default.jpg'" onload="atualizarIndicadorScrollModal()">
            ...
            ${temSeletorCores ? renderizarSeletorCores(materiaisFlatList) : ''}
            <div class="modal-tamanho-quantidade">
                ...
                <div class="tamanho-coluna">
                    <div class="tamanho-botoes">
                        ${botoesTamanho}
                    </div>
                    <div class="tamanho-descricao" id="tamanho-descricao">${textoTamanhoInicial}</div>
                </div>
            </div>
            <div class="modal-total" id="modal-total">
                Total: R$ ${totalInicial.toFixed(2)}
            </div>
    ...
    // Cria uma entrada no histórico para que o botão "voltar" (navegador/celular)
    // feche o popup em vez de sair da página
    if (!modalNoHistorico) {
        history.pushState({ produtoModal: true }, '');
        modalNoHistorico = true;
    }
    
    // Bloquear scroll da página
    bloquearScroll();

    // Garante que o popup sempre abre do topo e recalcula o indicador de
    // rolagem (seta + gradiente). Repete algumas vezes porque a altura real
    // só fica definitiva depois que a imagem/seletor de cores terminam de
    // carregar e a animação de abertura do popup termina.
    modalBody.scrollTop = 0;
    atualizarIndicadorScrollModal();
    setTimeout(atualizarIndicadorScrollModal, 150);
    setTimeout(atualizarIndicadorScrollModal, 400);
    
    // Configurar eventos de zoom após o modal ser aberto
    setTimeout(configurarZoom, 100);

    // Configurar o seletor de cores, se este produto tiver
    if (temSeletorCores) {
        setTimeout(configurarSeletorCores, 100);
    }
}
...
// Selecionar tamanho (P / M / G) no modal
function selecionarTamanho(letra) {
    if (!produtoModalAtual) return;

    const tamanho = obterTamanhos(produtoModalAtual).find(t => t.letra === letra);
    if (!tamanho || !tamanho.disponivel) return;

    tamanhoSelecionadoModal = tamanho;

    document.querySelectorAll('.tamanho-btn').forEach(btn => {
        btn.classList.toggle('selecionado', btn.dataset.tamanho === letra);
    });

    const descricaoEl = document.getElementById('tamanho-descricao');
    if (descricaoEl) descricaoEl.textContent = tamanho.medida || `Tamanho ${letra}`;

    atualizarPrecoTotal();
}

// Atualizar preço total no modal (preço do tamanho selecionado x quantidade)
function atualizarPrecoTotal() {
    const inputQuantidade = document.getElementById('quantidade-modal');
    const modalTotal = document.getElementById('modal-total');
    
    if (!inputQuantidade || !modalTotal) return;
    
    const quantidade = parseInt(inputQuantidade.value) || 1;
    const preco = tamanhoSelecionadoModal ? tamanhoSelecionadoModal.preco : 0;
...
function adicionarAoCarrinho(produtoId) {
    ...
    const precisaCor = produto.tipo === 'unico_material' && materiaisFlatList.length > 0;

    // Tamanho escolhido no modal (P por padrão)
    const tamanhoEscolhido = tamanhoSelecionadoModal;
    if (!tamanhoEscolhido || !tamanhoEscolhido.disponivel) {
        mostrarNotificacao('⚠️ Este adesivo não tem tamanho disponível no momento.');
        return;
    }

    // Se o produto exige escolha de cor e nenhuma cor foi detectada, avisa e cancela
    if (precisaCor && !corSelecionadaModal) {
        mostrarNotificacao('⚠️ Selecione uma cor antes de adicionar ao carrinho!');
        return;
    }

    const corEscolhida = precisaCor ? corSelecionadaModal : null;
    ...
    // Cada combinação produto + tamanho (+ cor, nos de cor única) é um item DISTINTO no carrinho.
    // Assim, o mesmo adesivo em P e em G nunca sobrescreve/soma um com o outro.
    const idBase = `${produto.id}-${tamanhoEscolhido.letra}`;
    const itemId = corEscolhida
        ? `${idBase}-${corEscolhida.categoriaId}-${corEscolhida.nome}`
        : idBase;
    ...
    if (itemExistente) {
        ...
        const novoItem = {
            id: itemId,
            nome: produto.cod + " | " + produto.nome + (corEscolhida ? ` (${corEscolhida.nome})` : ''),
            preco: tamanhoEscolhido.preco,
            imagem: produto.imagem,
            quantidade: quantidade,
            servico: 'Adesivo',
            tamanho: tamanhoEscolhido.letra,
            tamanhoMedida: tamanhoEscolhido.medida,
        };

        if (corEscolhida) {
            novoItem.cor = corEscolhida.nome;
            novoItem.tipoMaterial = corEscolhida.categoriaNome;
            novoItem.corCodigo = corEscolhida.codigo;
            novoItem.corTextura = corEscolhida.textura;
        }
    ...
    const sufixoCor = corEscolhida ? ` (${corEscolhida.nome})` : '';
    mostrarNotificacao(`✅ ${quantidade} x ${produto.nome}${sufixoCor} - ${tamanhoEscolhido.letra} adicionado ao carrinho!`);
    atualizarContadorCarrinho();
}

// Controle do histórico: o popup ocupa uma entrada no histórico do navegador
let modalNoHistorico = false;

// Fecha visualmente o popup (sem mexer no histórico)
...
// Fecha o popup por ação da interface (X, clique fora, adicionar ao carrinho)
function fecharModal() {
    if (modalNoHistorico) {
        // Remove a entrada do histórico; o handler de popstate fecha o popup
        history.back();
    } else {
        fecharModalVisual();
    }
...
// Botão voltar do navegador/celular: fecha o popup em vez de voltar de página
window.addEventListener('popstate', () => {
    if (modalNoHistorico) {
        modalNoHistorico = false;
        fecharModalVisual();
    }
});
...
function configurarEventListeners() {
    ...
    // ===== Indicador de rolagem do modal (seta + gradiente) =====
    // Registrado uma única vez aqui; o conteúdo do modal-body é substituído
    // a cada abertura, mas o próprio elemento modal-body nunca muda.
    if (modalBody) {
        modalBody.addEventListener('scroll', atualizarIndicadorScrollModal);
    }
    window.addEventListener('resize', atualizarIndicadorScrollModal);
}
