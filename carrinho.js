    ...
    carrinho.forEach(item => {
        ...
        html += `
            <div class="carrinho-item">
                <div class="item-imagem-wrapper">
                    <img src="${item.imagem}" 
                        alt="${item.nome}" 
                        style="width: 80px; height: 60px; object-fit: contain; background: #f9f9f9; border-radius: 4px; padding: 2px;">
                    ${item.cor ? `
                        <span class="item-cor-badge" 
                            style="background-image: url('${item.corTextura || ''}'); background-color: ${item.corCodigo || '#ccc'};" 
                            title="${item.tipoMaterial ? item.tipoMaterial + ' - ' : ''}${item.cor}"></span>
                    ` : ''}
                </div>
                <div class="item-info">
                    <h3>${item.nome}</h3>
                    ${item.tamanho ? `
                        <p class="item-tamanho">
                            <span class="item-tamanho-letra">${item.tamanho}</span>
                            ${item.tamanhoMedida ? `<span>${item.tamanhoMedida}</span>` : ''}
                        </p>
                    ` : ''}
...
window.finalizarCompra = async () => {
    ...
    try {
        ...
        const itensParaSalvar = carrinho.map(item => {
            ...
            // Tamanho escolhido no catálogo (P / M / G) e a medida correspondente
            if (item.tamanho && item.tamanho !== 'undefined') {
                itemBase.tamanho = item.tamanho;
                itemBase.tamanhoMedida = item.tamanhoMedida || '';
            }
        ...
        carrinho.forEach((item) => {
            ...
            switch(tipoProduto) {
                ...
                case 'adesivo':
                case 'Adesivo':
                    if (item.tamanho) mensagem += `Tamanho: ${item.tamanho}${item.tamanhoMedida ? ' (' + item.tamanhoMedida + ')' : ''}\n`;
                    if (item.tipoMaterial) mensagem += `Material: ${item.tipoMaterial}\n`;
                    else if (item.tipo) mensagem += `Material: ${item.tipo}\n`;
                    if (item.cor) mensagem += `Cor escolhida: ${item.cor}\n`;
